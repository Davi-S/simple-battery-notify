...
class BatteryDaemon:
    def __init__(self, config, device):
        self.config = config
        self.device = device
        self.last_percent = int(device.Percentage)
        self.last_state = int(device.State)

    def on_properties_changed(self, interface, changed, invalidated):
        if "Percentage" in changed or "State" in changed:
            current_percent = int(self.device.Percentage)
            current_state = int(self.device.State)
            time_str = get_time_string(self.device)
